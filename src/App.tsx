...
const DOCUMENT_COLUMNS =
  "id, project_id, file_name, mime_type, size_bytes, status, storage_path, extracted_supplier_name, extracted_date, extracted_amount_ht, extracted_amount_ttc, extracted_vat_amount, extracted_vat_rate, has_vat, extraction_confidence, extraction_notes, validated_at";
  ...
  return (
    <div className="doc-item">
      ...
      <div className="doc-actions">
        {d.validated ? (
          <span className="doc-status doc-validated">✓ Validé</span>
        ) : (
          <>
...
function DocumentsPage({ projects, docs, openDoc, openStoredDocument, analyzeDocument, analyzingId, openExpenseFromDoc, addLabel }: { projects: Project[]; docs: DocumentItem[]; openDoc: () => void; openStoredDocument: (document: DocumentItem) => Promise<void>; analyzeDocument: (d: DocumentItem) => void; analyzingId: string | null; openExpenseFromDoc: (d: DocumentItem) => void; addLabel: string }) {
  const pending = docs.filter((d) => !d.validated);
  return <main className="content"><div className="page-header"><div><h1>Documents IA</h1><p>Tickets et factures à traiter. Une fois validés en dépense, ils sortent de cette liste et restent rattachés à leur chantier.</p></div><button className="primary" onClick={openDoc}>{addLabel}</button></div><section className="panel"><h2>À traiter ({pending.length})</h2>{pending.length === 0 ? <p className="empty">Aucun document en attente. Tout est traité 🎉</p> : <div className="doc-list">{pending.map((d) => { const p = projects.find((x) => x.id === d.projectId); return <DocItem key={d.id} d={d} projectName={p?.name || "Chantier inconnu"} analyzing={analyzingId === d.id} analyzeDocument={analyzeDocument} openExpenseFromDoc={openExpenseFromDoc} openStoredDocument={openStoredDocument} />; })}</div>}</section></main>;
}
...
export default function App() {
  ...
  async function persistExpense(expense: Expense) {
    ...
    // Marque le document comme validé : il sort de la boîte "Documents IA"
    if (expense.documentId) {
      const { error: validateError } = await supabase
        .from("documents")
        .update({ validated_at: new Date().toISOString() })
        .eq("id", expense.documentId);
      if (validateError) {
        // La dépense est créée : on ne bloque pas, on signale seulement.
        console.warn(`Dépense créée, mais marquage du document impossible : ${validateError.message}`);
      }
    }
